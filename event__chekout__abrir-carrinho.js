    ...
    run: async (interaction, client) => {
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            if (db.has(customId)) {
                ...
                if (cargos.length > 0) {
                    ...
                    if (hasRole) {
                        return interaction.reply({ content: `${dbe.get(`13`)} | Você não pode comprar este produto porque tem um cargo proibido!`, flags: 64 });
                    }
                }

                if (produto.condições.cargo && !rolevery) {
                    return interaction.reply({ content: `${dbe.get(`13`)} | Você não tem o cargo necessário para comprar este produto!`, flags: 64})
                }
                if (!paumito) {
                    interaction.reply({ content: `${dbe.get(`13`)} | Canal logs privadas inválido!`, flags: 64})
                    return;
                }
                if (!de) {
                    interaction.reply({ content: `${dbe.get(`13`)} | Cargo staff inválido!`, flags: 64})
                    return;
                }
                if (!frango) {
                    interaction.reply({ content: `${dbe.get(`13`)} | Cargo cliente inválido!`, flags: 64})
                    return;
                }
                
                if (dbc.get(`pagamentos.sistema`) === "OFF") {
                    interaction.reply({ content: `${dbe.get(`13`)} | Sistema de vendas desligado!`, flags: 64})
                    return;
                }
                ...
                if (produto) {
                    ...
                    const msg = await interaction.reply({ content: `${dbe.get(`16`)} | Aguarde, estamos criando o carrinho.`, flags: 64})
                    ...
                    interaction.reply({ content: `${dbe.get(`13`)} | Produto não encontrado! Fale com o suporte do servidor.`, flags: 64})
                }
            }
        }
        ...
        if (interaction.isButton()) {
            ...
            if (customId.endsWith("_ativarnotify")) {
                ...
                if (findUser) {
                    interaction.reply({ content: `${dbe.get(`2`)} | Você ja está na lista de notificação deste produto!`, flags: 64})
                    return
                } else {
                    ...
                    interaction.reply({ content: `${dbe.get(`6`)} | Você foi adicionado à lista de notificação deste produto com sucesso!`, flags: 64})
                }
            }
        }
    }
